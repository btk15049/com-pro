...
bitset<212345> solve(vector<int64_t> a, vector<int64_t> b, int64_t k) {
    auto sum = ext::count(b, 1);
    vector<int64_t> na(a.size());
    {
        int64_t last = -1e10;
    ...
    ext::reverse(na);
    auto c = atcoder::convolution(na, b);
    c.resize(a.size());
    ext::reverse(c);
    c.resize(a.size() + 1u - b.size());
    bitset<212345> bs;
...
int main() {
    ...
    auto f = [&](char c) {
        for (int i : range(N)) a[i] = (s[i] == c ? 1 : 0);
        for (int i : range(M)) b[i] = (t[i] == c ? 1 : 0);
        return solve(a, b, K);
    };
    auto A = f('A');
    auto C = f('C');
