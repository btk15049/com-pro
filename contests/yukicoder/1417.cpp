...
ModInt dp1[112345][3][3];
bool vis1[112345];

ModInt dp2[112345][3][3];
bool vis2[112345];
...
void f1(size_t id) {
    ...
    const int cap = s[id] - '0';
    for (int v : range(1, cap)) {
        for (int i2 : range(3)) {
            for (int i5 : range(3)) {
                const ModInt from = dp2[id + 1][i2][i5];
                if (v == 2 || v == 6) {
                    dp1[id][min(i2 + 1, 2)][i5] += from;
                }
                else if (v == 4 || v == 8) {
                    dp1[id][min(i2 + 2, 2)][i5] += from;
                }
                else if (v == 5) {
                    dp1[id][i2][min(i5 + 1, 2)] += from;
                }
                else {
                    dp1[id][i2][i5] += from;
                }
    ...
    for (int v : range(max(cap, 1), cap + 1)) {
        for (int i2 : range(3)) {
            for (int i5 : range(3)) {
                const ModInt from = dp1[id + 1][i2][i5];
                if (v == 2 || v == 6) {
                    dp1[id][min(i2 + 1, 2)][i5] += from;
                }
                else if (v == 4 || v == 8) {
    ...
    for (int v : range(1, 10)) {
        for (int i2 : range(3)) {
            for (int i5 : range(3)) {
                const ModInt from = dp2[id + 1][i2][i5];
                if (v == 2 || v == 6) {
                    dp2[id][min(i2 + 1, 2)][i5] += from;
                }
                else if (v == 4 || v == 8) {
                    dp2[id][min(i2 + 2, 2)][i5] += from;
                }
                else if (v == 5) {
                    dp2[id][i2][min(i5 + 1, 2)] += from;
                }
                else {
                    dp2[id][i2][i5] += from;
                }
