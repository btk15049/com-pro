...
struct expr {
    using LeftHandSide = bitset<64>;
    LeftHandSide lhs;
    int rhs;
    expr() {}
    expr(LeftHandSide lhs, int rhs) : lhs(lhs), rhs(rhs) {}
    bool isLhs0() { return lhs.none(); }
    bool isInvalid() { return isLhs0() && rhs == 1; }
    string show() {
        vector<string> xs;
        for (int i : range(lhs.size())) {
            if (lhs[i]) {
                xs.push_back("x" + to_string(i));
            }
        }
        return join(xs, " + ") + " = " + to_string(rhs);
    }
};

expr operator^(expr l, expr r) { return expr(l.lhs ^ r.lhs, l.rhs ^ r.rhs); }

// Ax = y を解く
// 解なしのときはfalseが返る
bool gaussJordan(vector<expr>& exprList) {
    constexpr size_t termSize = expr::LeftHandSide().size();
    vector<vector<expr>> es(termSize);
    ...
    for (auto e : exprList) {
        ...
        es[e.lhs._Find_first()].push_back(e);
    }

    for (int i : range(termSize)) {
        if (es[i].empty()) {
            continue;
        }

        while (es[i].size() > 1u) {
            auto newExpr = es[i].front() ^ es[i].back();
            es[i].pop_back();
            ...
            es[newExpr.lhs._Find_first()].push_back(newExpr);
        }
    ...
    for (int i : range(termSize)) {
        ...
        exprList.push_back(es[i].front());
    }
    return true;
}

vector<int> getOneX(vector<expr>& exprList) {
    constexpr size_t termSize = expr::LeftHandSide().size();
    vector<int> x(termSize);

    for (auto& it : exprList) {
        x[it.lhs._Find_first()] = it.rhs;
    }
...
int main() {
    ...
    int N, M;
    cin >> N >> M;
    vector<bitset<64>> A(M);
    vector<bitset<64>> Y(M);
    for (int i : range(M)) {
        int num;
        cin >> num;
        for ([[maybe_unused]] int j : range(num)) {
            int b;
            cin >> b;
            A[i][b - 1] = true;
        }
    ...
    vector<bitset<64>> ret(N);
    for (int bi : range(30)) {
        vector<expr> es(M);
        for (int i : range(M)) {
            es[i] = expr(A[i], Y[i][bi] ? 1 : 0);
        }

        if (gaussJordan(es) == false) {
            cout << -1 << endl;
            return 0;
        }
        auto X = getOneX(es);
