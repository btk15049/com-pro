...
string JC;
string JJ;
int calc(char f, int m, char b, int x, int y) {
    int ret = numeric_limits<int>::max();
    chmin(ret, cost(f + CC.substr(0, m) + b, x, y));
    chmin(ret, cost(f + CJ.substr(0, m) + b, x, y));
    chmin(ret, cost(f + JC.substr(0, m) + b, x, y));
    chmin(ret, cost(f + JJ.substr(0, m) + b, x, y));
    return ret;
}
...
void solve(int caseNum) {
    ...
    for (int i : range(1, pos.size())) {
        ret += calc(s[pos[i - 1]], pos[i] - pos[i - 1] - 1, s[pos[i]], x, y);
    }
    cout << ret << endl;
}
