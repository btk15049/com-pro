...
int n, k;
ModInt dp[2][27][10000]; // dp[i % 2][穴の数][浮いてる数][2500]
using Table = decltype(dp[0]);
...
void update(Table& cr, Table& nx) {
    reset(nx);
    for (int in : range(26)) {
        const int out = in;
        for (int cost : range(k + 1)) {
            // 自分で使う
            if (cost + in + out <= 2500)
                nx[in][cost + in + out] += cr[in][cost];
            if (in > 0 && out > 0) {
                // 自分は左へ
                // 浮いてるのを埋める
                const int ni = in - 1;
                const int no = out - 1;
                const int nc = cost + ni + no;
                if (nc <= 2500) nx[ni][nc] += in * out * cr[in][cost];
            }
            if (in > 0) {
                // 自分は左へ
                // 自分の穴はそのまま
                ...
                const int no = out;
                const int nc = cost + ni + no;
                if (nc <= 2500) nx[ni][nc] += in * cr[in][cost];
            }
            if (in < 26) {
                // 自分は右へ
                // 自分の穴はそのまま
                ...
                const int no = out + 1;
                const int nc = cost + ni + no;
                if (nc <= 2500) nx[ni][nc] += cr[in][cost];
            }
...
int main() {
    ...
    dp[0][0][0] = 1;
    for (int i = 0; i < n; i++) {
        update(dp[i % 2], dp[1 - i % 2]);
    }
    const auto ret = *dp[n % 2][0][k];
    cout << ret << endl;
    return 0;
}