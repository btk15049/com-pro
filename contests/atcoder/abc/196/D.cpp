...
int main() {
    ...
    for (int i : range(R)) {
        // 横置き
        for (int j : range(C - 1)) {
            for (int lt : !range(1, A + 1)) {
                for (int _b : range(1 << C)) {
                    bitset<4> b(_b);
                    if (b[j] || b[j + 1]) continue;
                    b[j]     = true;
                    b[j + 1] = true;
                    dp[lt - 1][b.to_ulong()] += dp[lt][_b];
                }
            }
        }
        // 1個置き
        for (int j : range(C)) {
            for (int lt : range(0, A + 1)) {
                for (int _b : range(1 << C)) {
                    bitset<4> b(_b);
                    if (b[j]) continue;
                    const int used    = i * C + b.count();
                    const int used_lt = (A - lt);
                    const int used_ht = used - used_lt * 2;
                    const int ht      = B - used_ht;
                    if (ht == 0) continue;
                    b[j] = true;
                    dp[lt][b.to_ulong()] += dp[lt][_b];
                }
            }
        }
        // 縦置き
        if (i != R - 1) {
            ...
            for (int lt : range(0, A + 1)) {
                for (int _b : range(1 << C)) {
                    bitset<4> b(_b);
                    if (C - b.count() > lt) continue;
                    nx[lt - (C - b.count())][((1 << C) - 1) ^ b.to_ulong()] +=
                        dp[lt][_b];
                }
            }
            swap(nx, dp);
