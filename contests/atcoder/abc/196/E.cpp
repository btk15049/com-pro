...
int main() {
    ...
    int64_t g = 0;
    auto xi   = ext::iota(Q);
    ext::sort(xi, [&](int l, int r) { return x[l] < x[r]; });
    deque<pair<int64_t, int>> y;
    for (int i : range(Q)) {
        if (y.empty() || y.back().first != x[xi[i]]) {
            y.emplace_back(x[xi[i]], 1);
        }
        else {
            y.back().second++;
        }
    ...
    for (int i : range(N)) {
        switch (f[i].t) {
            case 1:
                ...
                break;
            case 2:
                if (y.front().first + g < f[i].a) {
                    auto p = y.front();
                    y.pop_front();
                    p.first += f[i].a - g - p.first;
                    while (!y.empty() && y.front().first + g < f[i].a) {
                        p.second += y.front().second;
                        y.pop_front();
                    }
                    y.push_front(p);
                }
                break;
            case 3:
                if (y.back().first + g > f[i].a) {
                    auto p = y.back();
                    y.pop_back();
                    p.first -= p.first + g - f[i].a;
                    while (!y.empty() && y.back().first + g > f[i].a) {
                        p.second += y.back().second;
                        y.pop_back();
                    }
                    y.push_back(p);
                }
