...
inline uint64_t key(uint64_t a, uint64_t b) { return (a << 32) + b; }
int main() {
    /* write here */
    ...
    auto* _v = new unordered_map<uint64_t, int>();
    auto& v  = *_v;
    for (uint64_t i : range(N)) {
        int a;
        cin >> a;
        v.emplace(i, a);
    }
    for (uint64_t i : range(1, N)) {
        int a;
        cin >> a;
        v.emplace(i << 32, a);
    }
    ...
    for (uint64_t i : range(1, q)) {
        if (i >= N) continue;
        for (uint64_t j : range(q, N)) {
            int a = v[key(i - 1, j)];
            int b = v[key(i, j - 1)];
            int c = mex[a][b];
            v.emplace(key(i, j), c);
    ...
    vector<int64_t> cnt(3);

    for (const auto& [k, m] : v) {
        const auto i = k >> 32;
        const auto j = k & uint32_t(-1);
        cnt[m]++;
        if (min(i, j) == q - 1) {
            cnt[m] += N - 1 - max(i, j);
        }
    ...
    cout << join(cnt.begin(), cnt.end(), ' ') << endl;
    return 0;
}
