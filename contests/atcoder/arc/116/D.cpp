...
int main() {
    ...
    int N, M;
    cin >> N >> M;
    vector<atcoder::modint998244353> dp(M + 1);
    dp[0] = 1;
    int p = 1;
    for (int i : range(15)) {
        vector<atcoder::modint998244353> b(M + 1);
        for (int j = 0; j * p <= M; j += 2) {
            // cerr << N << " " << j << " " << *factorial::combination(N, j)
            //     << endl;
            b[j * p] = *factorial::combination(N, j);
        }
        dp = atcoder::convolution(dp, b);
        dp.resize(M + 1);
        p *= 2;
        // cerr << dp[2].val() << endl;
    }
