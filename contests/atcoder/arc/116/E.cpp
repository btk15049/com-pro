...
struct info {
    int points;
    int needCoverDistance;
    int distanceToNearestPoint;
    info() {
        points                 = 0;
        needCoverDistance      = -10 * N;
        distanceToNearestPoint = 10 * N;
    }
};

info dfs(int v, int d) {
    auto& t = *tree;
    info i;
    vector<info> cs;
    for (int u : t.Ns(v)) {
        cs.push_back(dfs(u, d));
    }
    for (auto ci : cs) {
        i.points += ci.points;
        chmin(i.distanceToNearestPoint, ci.distanceToNearestPoint + 1);
    }
    if (d < i.distanceToNearestPoint) {
        i.needCoverDistance = 0; // v 自身がカバーされていない
    }
    for (auto ci : cs) {
        chmax(i.needCoverDistance, ci.needCoverDistance + 1);
    }
    if (d - i.distanceToNearestPoint >= i.needCoverDistance) {
        i.needCoverDistance = -10 * N;
    }
    if (i.needCoverDistance >= d) {
        i.needCoverDistance      = -10 * N;
        i.distanceToNearestPoint = 0;
        i.points++;
    }
    return i;
}
...
bool solve(int d) {
    ...
    const auto i = dfs(0, d);
    int need     = i.points;
    if (i.needCoverDistance >= 0) {
        need++;
    }
    return need <= K;
}
...
int main() {
    ...
    int ok = math::ceil(N, K);
    int ng = 0;
    Graph<> g(212345, 412345);
    for ([[maybe_unused]] int i : range(N - 1)) {
        int a, b;
        cin >> a >> b;
        a--;
        b--;
