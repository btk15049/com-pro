...
int main() {
    ...
    priority_queue<int64_t, vector<int64_t>, greater<int64_t>> rs;
    priority_queue<int64_t> ls;
    int64_t rb = 0;
    int64_t lb = 0;
    ...
    for (int i : range(1, N)) {
        rb += r[i - 1] - l[i - 1];
        lb -= r[i] - l[i];
        const int64_t lm = lb + ls.top();
        const int64_t rm = rb + rs.top();
        ...
        while (ls.size() < rs.size()) {
            ls.push(rs.top() + rb - lb);
            rs.pop();
        }

        while (ls.size() > rs.size()) {
            rs.push(ls.top() + lb - rb);
            ls.pop();
        }


        const int64_t clm = lb + ls.top();
        ret += maxOf(clm - rm, lm - clm, int64_t(0));
        ret += abs(l[i] - clm);
    }
    cout << ret << endl;
    return 0;
