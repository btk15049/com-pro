...
int nx[1123456][4];
string s;
inline int getXor(int l, int r) { return xs[r] ^ xs[l]; }
inline bool isValid(int l, int r) { return cs[r][v[l]] - cs[l][v[l]] != r - l; }
...
int main() {
    ...
    for (const int i : range(N)) {
        ...
        if (!isValid(i, N)) {
            ...
            else if (i > 0 && isValid(0, N) && getXor(i, N) != 0) {
                ret += dp[i];
            }
        }
        else {
        ...
        if (isValid(i, N)) {
            for (const int j : range(1, 4)) {
                if (j == getXor(i, i + 1)) continue;
                const int lb = [&]() {
                    int ok = N;
                    int ng = i;
                    while (ok - ng > 1) {
                ...
                // k>lb を満たす [i, k) である必要がある
                const int target = getXor(0, i) ^ j;
                const int k      = nx[lb][target];
                if (k == N) {
                    continue;
                }
                dp[k] += dp[i];
