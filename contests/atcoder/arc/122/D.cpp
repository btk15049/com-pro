...
bool validate(vector<B> &t, vector<B> &f, B x, int k) {
    if (k == -1) return false;
    if (t.empty() || f.empty()) return false;
    vector<B> tt;
    vector<B> tf;
    vector<B> ft;
    vector<B> ff;
    ...
    if (x.test(k)) {
        if (tt.size() > 0u && ft.size() > 0u) return true;
        if (ff.size() > 0u && tf.size() > 0u) return true;
        return validate(t, f, x, k - 1);
    }
    else {
        if (validate(tt, ft, x, k - 1)) return true;
        if (validate(tf, ff, x, k - 1)) return true;
        return false;
    }
}

bool validate(vector<B> &a, B x, int k) {
    vector<B> t;
    vector<B> f;
    for (auto &it : a) {
        if (it.test(k)) {
    ...
    // cerr << t.size() << " " << f.size() << endl;
    if (t.size() % 2 == 0 && f.size() % 2 == 0) {
        return true;
    }
    else {
        return validate(t, f, x, k - 1);
...
bool solve(uint32_t x) {
    ...
    uint32_t uMask = numeric_limits<uint32_t>::max() - lMask;
    vector<vector<B>> val;
    uint32_t p = numeric_limits<uint32_t>::max();
    for (int i : range(N)) {
        auto u = uMask & A[i];
        if (u != p) {
            val.push_back(vector<B>());
        }
        val.back().emplace_back(lMask & A[i]);
        p = u;
    }
    // cerr << x << " " << msb;
    // for (auto &it : val) {
