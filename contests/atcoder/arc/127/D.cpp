...
inline void clr(decltype(cnt) ptr) {
    for (int i : range(18)) {
        ptr[i][0] = 0;
        ptr[i][1] = 0;
    }
...
void f(const vector<bit>& A, const vector<bit>& B, int pos) {
    if (A.empty()) return;
    if (pos == -1) {
        // A[i]^B[i] == A[j]^B[j]
        // すなわち
        ...
        for (auto& it : A) {
            for (int i : range(18)) {
                const int64_t k = (1 << i) * cnt[i][it.test(i) ^ true];
                ans += k;
            }
        }
        ret += ans / 2;
    ...
    vector<bit> SA, SB, TA, TB;
    vector<vector<int>> S(2), T(2);
    for (int i : range(A.size())) {
        const auto C = A[i] ^ B[i];
        if (C.test(pos) == false) {
            SA.push_back(A[i]);
            SB.push_back(B[i]);
            S[A[i].test(pos)].push_back(i);
        }
        else {
            TA.push_back(A[i]);
            TB.push_back(B[i]);
            T[A[i].test(pos)].push_back(i);
        }
    }
    f(SA, SB, pos - 1);
    f(TA, TB, pos - 1);
    if (SA.empty() || TA.empty()) return;
    ...
    for (int id : S[0]) {
        for (int i : range(18)) {
            ret += (1 << i) * tac[0][i][A[id].test(i) ^ true];
            ret += (1 << i) * tbc[1][i][B[id].test(i) ^ true];
        }
    }
    for (int id : S[1]) {
        for (int i : range(18)) {
            ret += (1 << i) * tac[1][i][A[id].test(i) ^ true];
            ret += (1 << i) * tbc[0][i][B[id].test(i) ^ true];
        }
    }
