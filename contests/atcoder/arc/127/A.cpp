...
int64_t f(int64_t n, int64_t r) { return min(n, r - 1); }

int64_t f(int64_t n, int64_t l, int64_t r) { return f(n, r) - f(n, l); }
...
int main() {
    ...
    cin >> N;
    int64_t ret = 0;
    for (int len : range(1, 17)) {
        {
            auto lb = stoll(string(len, '1'));
            ret += len * f(N, lb, lb + 1);
        }
        for (int v : range(1, len)) {
            for (int k : range(0, 10)) {
                if (k == 1) continue;
                auto prefix = string(v, '1') + to_string(k);
                auto lb     = prefix + string(len - v - 1, '0');
                auto ub     = prefix + string(len - v - 1, '9');
                int64_t u   = f(N, stoll(lb), stoll(ub) + 1);
                // if (u > 0) cerr << lb << " " << u << endl;
                ret += v * u;
            }
        }
    }
