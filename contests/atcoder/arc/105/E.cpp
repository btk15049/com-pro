...
/*
- 4n+1 <-> 4n+3 勝者固定 == カットの辺は常に偶数
- 4n+0 <-> 4n+2 奇数同士の連結成分をくっつけるたび最終的な偶奇入れ替わる
*/
...
void solve() {
    ...
    int c1 = c[uf.find(0)] % 2;
    int cN = c[uf.find(N - 1)] % 2;

    auto tot = int64_t(N) * (N - 1) / 2;
    if (o % 2 == 1) {
        if ((tot - M) % 2) {
            cout << "First\n";
        }
        ...
        if ((tot - M) % 2) {
            // 先手は最終的に奇数頂点の連結成分を無くせればOK
            // ただし、1とNを含む連結成分がそれぞれ奇数になって、他に奇数頂点を含む頂点が存在しない場合世界終了
            // 初期状態でc1%2==1 かつ cN%2 ==1 なら負け
            // そうでないなら勝ち
            if (c1 * cN == 1) {
                cout << "Second\n";
            }
