...
namespace {
    ...
    auto set_leaf = [](node &v, const int id) {
        v.a    = 1;
        v.ret  = 0;
        v.b    = 0;
        v.size = 0;
        if (id < el.size()) {
            v.size = el[id].ed - el[id].bg;
        }
    };

    auto set_segment = [](node &v, const int l, const int r) {
        v.a    = 1;
        v.ret  = 0;
        v.b    = 0;
        v.size = 0;
        if (r <= (int)el.size()) {
            v.size = el[r - 1].ed - el[l].bg;
        }
    };

    auto segment_sum = [](node &v, [[maybe_unused]] const int l,
                          [[maybe_unused]] const int r) {
        return (v.a * v.ret) + v.b * ModInt((long long)(v.size));
    };

    auto merge_segment = [](ModInt l, ModInt r) { return l + r; };

    auto mul_update = [](node &v, ModInt l) {
        v.a *= l;
        v.b *= l;
    };

    auto add_update = [](node &v, ModInt l) { v.b += l; };


    auto lazy = [](node &v, node &l, node &r) {
        if (*v.a != 1) {
            l.a *= v.a;
            l.b *= v.a;
            r.a *= v.a;
    ...
    template <typename Node, typename RET, typename CalcSegmentResultFunction,
              typename MergeFunction, typename InitSegmentFunction,
              typename InitLeafFunction, typename LazyUpdateFunction>
    struct SegmentTree {
        int size;
        Node *seg;
        CalcSegmentResultFunction get_single_segment;
        MergeFunction get_merged_segment;
        LazyUpdateFunction lazy_update;
        void init(int l, int r, InitSegmentFunction init_segment,
                  InitLeafFunction init_leaf, int k = 0) {
            auto &v = seg[k];
            init_segment(v, l, r);
            if (r - l == 1) {
                //葉の時の処理
        ...
        SegmentTree(int n, CalcSegmentResultFunction gss, MergeFunction gms,
                    InitSegmentFunction sis, InitLeafFunction sil,
                    LazyUpdateFunction lu)
            : get_single_segment(gss),
              get_merged_segment(gms),
              lazy_update(lu) {
            size = 1;
            while (size < n) size *= 2;
            seg = new Node[size * 2 + 10];
            init(0, size, sis, sil);
        }
...
#define RQ a, b, k * 2 + 2, m, r
        ...
        template <typename UpdateFunction, typename Q>
        void update(int a, int b, int k, int l, int r,
                    UpdateFunction update_segment, Q value) {
            if (r <= a || b <= l) return;
            if (a <= l && r <= b) {
                ...
                if (r - l > 1)
                    lazy_update(seg[k], seg[2 * k + 1], seg[2 * k + 2]);
                int m = (l + r) / 2;
        ...
        template <typename UpdateFunction, typename Q>
        void update_segment(int a, int b, UpdateFunction update_segment,
                            Q value) {
            update(a, b, 0, 0, size, update_segment, value);
        }

        template <typename UpdateFunction, typename Q>
        void update_element(int a, UpdateFunction update_segment, Q value) {
    ...
    //     int n = 100;
    //     /* init */
    //     SegmentTree<node, int> rmq(n, segment_min, merge_segment,
    //     set_segment,
    //                                set_leaf);
    //
    //     /* set */
...
int main() {
    ...
    std::sort(latte, latte + malta);
    malta = unique(latte, latte + malta) - latte;

    el.reserve(malta);
    for (int i : range(malta - 1)) {
        el.push_back(El{.bg = latte[i], .ed = latte[i + 1]});
    }

    SegmentTree<node, ModInt, decltype(segment_sum), decltype(merge_segment),
                decltype(set_segment), decltype(set_leaf), decltype(lazy)>
        seg(el.size(), segment_sum, merge_segment, set_segment, set_leaf, lazy);

    seg.update_segment(0, lower_bound(latte, latte + malta, A[0]) - latte,
                       add_update, ModInt(1));
    // 1 0
    // 1 2
    // 3 0
    for (int i : range(1, N)) {
        const int p = lower_bound(latte, latte + malta, A[i]) - latte;
        ModInt sum  = seg.get(0, el.size());
        seg.update_segment(0, p, mul_update, ModInt(0) - 1);

        if (p < el.size()) {
            seg.update_segment(p, el.size(), mul_update, ModInt(0));
        }
        seg.update_segment(0, p, add_update, sum);
        // cerr << *sum << " " << *seg.get(0, el.size()) << endl;
    }
    cout << *seg.get(0, el.size()) << endl;
    return 0;
}
