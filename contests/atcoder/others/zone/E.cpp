...
int64_t D[1123][1123][2];

int64_t lc(int& r, int& c) {
    c--;
    return A[r][c];
}
int64_t rc(int& r, int& c) {
    c++;
    return A[r][c - 1];
}
int64_t dc(int& r, int& c) {
    r++;
    return B[r - 1][c];
}
int main() {
    ...
    using T               = tuple<int64_t, int, int, int>;
    ...
    while (!que.empty()) {
        auto t = que.top();
        que.pop();
        const auto [d, r, c, s] = t;
        if (D[r][c][s] < d) continue;
        if (s == 0) {
            if (c > 0) {
                int nr     = r;
                int nc     = c;
                int64_t nd = d + lc(nr, nc);
                if (chmin(D[nr][nc][s], nd)) {
                    que.emplace(nd, nr, nc, s);
                }
            }
            if (c + 1 < C) {
                int nr     = r;
                int nc     = c;
                int64_t nd = d + rc(nr, nc);
                if (chmin(D[nr][nc][s], nd)) {
                    que.emplace(nd, nr, nc, s);
                }
            }
            if (r + 1 < R) {
                int nr     = r;
                int nc     = c;
                int64_t nd = d + dc(nr, nc);
                if (chmin(D[nr][nc][s], nd)) {
                    que.emplace(nd, nr, nc, s);
                }
            }
            if (chmin(D[r][c][1 - s], D[r][c][s] + 1)) {
                que.emplace(D[r][c][1 - s], r, c, 1 - s);
            }
        }
        else {
            if (r > 0) {
                int nr     = r - 1;
                int nc     = c;
                int64_t nd = D[r][c][s] + 1;
                if (chmin(D[nr][nc][s], nd)) {
                    que.emplace(nd, nr, nc, s);
                }
            }
            if (chmin(D[r][c][1 - s], D[r][c][s])) {
                que.emplace(D[r][c][1 - s], r, c, 1 - s);
            }
        }
    }
    cout << D[R - 1][C - 1][0] << endl;
