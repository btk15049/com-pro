...
namespace drken {
    ...
    const int MAX_COL = 20; // to be set appropriately
    struct BitMatrix {
        int H, W;
...
int main() {
    ...
    for (int i : range(1, N)) {
        ...
        if (p == n) continue;
        choosed.push_back(i);
        drken::BitMatrix bm(choosed.size(), n);
        for (int j : range(choosed.size())) {
            bitset<20> b(choosed[j]);
            for (int k : range(20)) {
                bm[j][k] = b[k];
            }
        }
        if (!chmax(p, drken::GaussJordan(bm))) {
            choosed.pop_back();
        }
    }
    cerr << p << " " << n << endl;
