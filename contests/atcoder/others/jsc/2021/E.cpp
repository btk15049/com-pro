...
int main() {
    ...
    cin >> k >> s;
    n               = s.size();
    const int l0Len = calc(k, n);
    cerr << s.substr(0, l0Len) << endl;

    set<int> l0;
    for (int i : range(l0Len)) {
        l0.insert(uf.find(i));
    }
    int64_t ret = 0;
    ...
    if (l0Len != 0) {
        ...
        vector<vector<pair<char, int>>> rr(n);

        for (auto &i : l0) {
            map<char, int> cnt;
            int ans = 0;
            for (auto i : range('a', 'z' + 1)) {
                cnt[i] = 0;
            }
            for (auto p : pos[i]) {
                cnt[s[p]]++;
            ...
            for (auto [k, v] : cnt) {
                rr[i].emplace_back(k, pos[i].size() - v);
            }
            ext::sort(rr[i], [](pair<char, int> lhs, pair<char, int> rhs) {
                return lhs.second < rhs.second;
            });
        }
        for (int i : range(l0Len)) {
            int p = uf.find(i);
            x.push_back(rr[i].front().first);
            ret += rr[i].front().second;
        }
        string y = x;
        ext::reverse(y);
        cerr << x << endl;
        ...
        if (x == y) {
            ...
            for (int i : range(l0Len / 2)) {
                int p = uf.find(i);
                int q = uf.find(l0Len - i - 1);
                for (auto pq : {p, q}) {
                    chmin(ret, base - rr[pq][0].second + rr[pq][1].second);
                }
            }
        }
    }
