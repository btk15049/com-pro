...
int main() {
    ...
    vector<int64_t> B(M, 0);
    BinaryIndexedTreePreset::Long::Sum::Preset As(malta + 10);
    BinaryIndexedTreePreset::Long::Sum::Preset Bs(malta + 10);
    BinaryIndexedTreePreset::Long::Sum::Preset Ac(malta + 10);
    BinaryIndexedTreePreset::Long::Sum::Preset Bc(malta + 10);
    Ac.add(0, N);
    Bc.add(0, M);
    ...
    for (int i : range(Q)) {
        if (T[i] == 1) {
            {
                int z = lower_bound(latte, latte + malta, A[X[i]]) - latte;
                cur -= latte[z] * Bc.sum(0, z + 1);
                cur -= Bs.sum(z + 1, malta);
                As.add(z, -latte[z]);
                Ac.add(z, -1);
            }
            A[X[i]] = Y[i];
            ...
            {
                int z = lower_bound(latte, latte + malta, B[X[i]]) - latte;
                cur -= latte[z] * Ac.sum(0, z);
                cur -= As.sum(z, malta);
                Bs.add(z, -latte[z]);
                Bc.add(z, -1);
            }
