...
int main() {
    ...
    int64_t sum = accumulate(A, A + N, int64_t(0));
    int64_t ret = Y * max(math::ceil(sum, K), A[N - 1]);
        ...
        while (ok - ng > 1) {
            ...
            for (int i : range(N)) {
                if (A[i] <= mid) {
                    ...
                    // A[i] - nK <=mid となる最小のn
                    // ceil(A[i]-mid, K)
                    s += max(int64_t(0), A[i] - math::ceil(A[i] - mid, K) * K);
                }
            }
            if (s >= 112345 * K) {
                ok = mid;
    ...
    for (int i : range(N)) {
        if (A[i] <= cap) {
            ...
            // A[i] - nK <=mid となる最小のn
            // ceil(A[i]-mid, K)
            const auto n = math::ceil(A[i] - cap, K);
            xc += n;
            A[i] -= n * K;
        }
        if (A[i] > 0) {
    ...
    while (!que.empty()) {
        int64_t p = que.top();
        chmin(ret, xc * X + max(math::ceil(cur, K), p) * Y);
        que.pop();
        xc++;
        cur -= p;
        p -= K;
