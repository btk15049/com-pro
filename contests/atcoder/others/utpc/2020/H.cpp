...
int main() {
    ...
    vector<string> S(R);
    cin >> S;
    vector<vector<int>> rc(R, vector<int>(2, 0));
    vector<vector<int>> cc(C, vector<int>(2, 0));
    for (int i : range(R)) {
        for (int j : range(C)) {
            const int c = S[i][j] == '#';
            rc[i][c]++;
    ...
    while (qr.size() + qc.size() > 0u && RR != 0 && CC != 0) {
        if (qr.size() > qc.size()) {
            const int r = qr.front();
            qr.pop();
            RR--;
            const int col = rc[r][1] > 0;
            for (int i : range(C)) {
                cc[i][col]--;
                if (cc[i][col] == 0 && cUsed[i] == false) {
                    qc.push(i);
                    cUsed[i] = true;
                }
            }
            ...
            const int c = qc.front();
            qc.pop();
            CC--;
            const int col = cc[c][1] > 0;
            for (int i : range(R)) {
                rc[i][col]--;
                if (rc[i][col] == 0 && rUsed[i] == false) {
                    qr.push(i);
                    rUsed[i] = true;
                }
            }
